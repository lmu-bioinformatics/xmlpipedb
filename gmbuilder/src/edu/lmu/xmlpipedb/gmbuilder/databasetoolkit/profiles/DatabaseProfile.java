...
public abstract class DatabaseProfile extends Profile {
	...
	/**
	 * @author Joey J. Barrett
	 * Stolen from the internet.  Used to sort
	 * the display order.
	 */
	...
	//Static Variables
	public static enum DisplayOrder {alphabetical};
	public static enum SystemType {Proper, Improper, Primary};
	...
	static {
		try {
			...
			PreparedStatement ps = ConnectionManager.getGenMAPPTemplateDBConnection().prepareStatement(
					"select System, SystemCode, Misc from Systems");
	        ...
	        while(result.next()) {
	        	...
	        	templateDefinedSystemTables.put(system, misc == null ? 
	        			SystemType.Proper : misc.contains("|I|") ? 
	...
	/**
	 * Returns whether this profile is available within the
	 * given connection.  A sub-class must define exactly
	 * what needs to be verified to be a valid X-centric
	 * database.
	 * 
	 * @param connection
	 * @return
	...
	/**
	 * Returns whether this profile matches any other requirements
	 * for the X-centric database.  This is place to check for
	...
	/**
	 * Returns the available species profiles contained 
	 * in this database.
	 * @return
	 */
	 ...
	 * @return
	 */
	public DisplayOrder[] getAvailableDisplayOrders() {
		return DisplayOrder.values();
	}
	
	/**
	 * Returns the available system tables found in 
	 * this database.  It is the difference between
	 * the tables defined in the template file and the
	...
	public Map<String, SystemType> getAvailableSystemTables() {
		...
		for(String systemTableName : templateDefinedSystemTables.keySet()) {
			if(systemTablesFound.contains(systemTableName)) {
				systemTablesAvailable.put(systemTableName, 
						templateDefinedSystemTables.get(systemTableName));
			}
		}
	...
	/**
	 * Returns a map of the database specific system tables.
	 * For an example of how to do this look at the sub-classes
	 * of this class.
	 * 
	 * @return
	 */
	...
	/**
	 * Returns the relationship tables based on the chosen
	 * system tables in the export wizard.  It uses the 
	 * predefined definition for relationship table combinations.
	 * 
	 * @return
	 */
		...
		for(Entry<String, SystemType> systemTable1 : systemTables.entrySet()) {
			if(systemTable1.getValue() == SystemType.Proper) {
				for(Entry<String, SystemType> systemTable2 : systemTables.entrySet()) {
					if(!systemTable1.getKey().equals(systemTable2.getKey()) &&
							!relationshipTablesAvailable.contains(systemTable2.getKey() + "-" + systemTable1.getKey())) {			
						relationshipTablesAvailable.add(relationshipTablesAvailable.size() == 0 ? 0 : relationshipTablesAvailable.size(), 
								systemTable1.getKey() + "-" + systemTable2.getKey());
					}
				}
			}
		}
	...
	/**
	 * Returns the MODSystem for this X-centric
	 * database.
	 * @return
	 */
	...
	/**
	 * Returns the display order of the chosen
	 * system tables using the display order
	 * option chosen in the export wizard.
	 * @return
	 */
	protected abstract String getDisplayOrder();
	
	/**
	 * Returns the GenMAPP database chosen
	...
	/**
	 * Returns the alternate connection chosen
	 * in the export wizard.
	...
	/**
	 * Returns the associations file chosen in the
	 * export wizard.
	 * 
	 * @return
	 */
	...
	/**
	 * Returns the primary system table for this
	 * X-centric database.  The primary system
	 * table must have a system type of "Primary".
	 * 
	 * @return
	 */
	...
	/**
	 * Returns a map of the chosen system tables
	 * from the export wizard.
	 * 
	 * @return
	 */
	...
	/**
	 * Sets this profiles main properties chosen in the
	 * export wizard.
	 * 
	 * @param owner
	 * @param version
	 ...
	 * @param notes
	 */
	public void setProfileProperties(String owner, Date version, 
			String modSystem, String speciesCustomizedName, Date modify, 
			DisplayOrder displayOrder, String notes) {
		
		this.owner = owner;
		this.version = version;
		this.modSystem = modSystem;
		speciesProfile.setCustomizedName(speciesCustomizedName);
		this.modify = modify;
	...
	/**
	 * Sets the connections and associations file 
	 * chosen in the export wizard.
	 * 
	 * @param genMAPPDatabase
	 * @param connectionConfiguration
	 * @param associationsFile
	 */
	public void setDatabaseProperties(File genMAPPDatabase, 
			ConnectionConfiguration connectionConfiguration, File associationsFile) {
	...
	/**
	 * Sets the table properties, specifically the system tables
	 * (both proper and improper) from the export wizard.
	...
	/**
	 * Returns table managers associated with the first pass through the 
	 * tables to be created.  It also updates the export wizard to 
	 * which tables it is preparing.
	 * 
	 * @return
	 * @throws Exception
	 */
	...
	/**
	 * Returns table managers associated with the second pass 
	 * through the tables to be created.  This is to be defined
	 * by a X-centric database.
	 * 
	 * @return
	 * @throws SQLException
	 * @throws Exception
	...
	private TableManager getInfoTableManager() throws Exception {
		...
		tableManager = new TableManager(null, new String[] {});
	...
	/**
	 * Prepares the relations TableManager for this database.  If
	 * a species specific change is required the 
	 * getRelationTableManagerCustomization() function is called.
	 * 
	 * @return
	 * @throws Exception
	 */
	private TableManager getRelationsTableManager() throws Exception {
		...
		tableManager = new TableManager(null, new String[] {"SystemCode", "RelatedCode"});
		...
		for(String relationTable : relationshipTables) {
			...
			if(speciesProfile.getSpeciesSpecificSystemTables().containsKey(systemTable1) |
				...
				tableManager.submit("Relations", QueryType.insert, new String[][] {
						...
						{"Type", systemTable1.equals(getPrimarySystemTable()) ||
							systemTable2.equals(getPrimarySystemTable()) ? "Direct" : "Inferred"},
						{"Source", ""}
				});
			}
		}
	...
	/**
	 * This function must be implemented by an 
	 * X-centric database and should the return
	 * the systems table manager associated with 
	 * that database. 
	 *  
	 * @return
	 * @throws Exception
	...
	/**
	 * This function must be implemented by an
	 * X-centric database and should return a 
	 * TableManager with the primary system table
	 * information associated with that database.
	 * 
	 * @return
	 * @throws Exception
	 */
	...
	/**
	 * This function must be implemented by an X-centric
	 * database and should return a TableManager with all
	 * system tables.
	 * 
	 * @return
	 * @throws Exception
	...
	/**
	 * This function must be implemented by an X-centric
	 * database and should return a TableManager List
	 * with all the relationship tables for the database.
	 * 
	 * @return
	 * @throws Exception
	 */
	...
	/**
	 * Creates the OriginalRowCounts TableManager.  This function
	 * should be fixed to query the database for which tables have
	 * actually been created rather than the few hardcoded values 
	 * and a compiling of the other tables assumed created.
	 * 
	 * @return
	 * @throws Exception
	 */
	...
	/**
	 * Returns the chosen export connection from the
	 * export wizard.
	 * 
	 * @return
	 * @throws Exception
