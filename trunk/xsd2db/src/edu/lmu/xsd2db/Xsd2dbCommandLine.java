...
public class Xsd2dbCommandLine {
	...
	private final String usage 	= "usage: program_name [--" + outputDirectory + "=dirname] "
										  + "[--" + bindings + "=filename] [[-" + updateXSD + "] --" + xsdURL +"=url] [-" + help +"]";
	private final String helpMsg	= 	"--" + outputDirectory + "=dirname -- " +
											"The directory when generating the source code and file; defaults to dg-gen\n" +
											"--" + bindings+ "=filename       -- " +
											"The binding file used when generation the data base source code and files\n" +
											"                             for the time. Defaults to standard binding file supplied bu XSD-To-DB\n" +
											"--" + xsdURL + "=url              -- " +
											"The URL of the XSD to convert\n" +
											"-" + updateXSD + "                -- " +
	 ...
	 * the database source output directory and
	...
	public void parse(String[] args)   {
		...
		dbSrcDir = new File(line.hasOption(outputDirectory)
			? line.getOptionValue(outputDirectory) : "db-gen");

		bindingsFile = line.hasOption(bindings)
			? line.getOptionValue(bindings) : "";
		...
		if(line.hasOption(updateXSD) && xsdurl.equals("")) {
			printErrorMsgAndExit(usage + "\n\n--" + xsdURL + 
					"=url must be specified when using -" + updateXSD);
			
		}

		if (!dbSrcDir.isDirectory() || (dbSrcDir.list().length == 0)) {
			if (xsdurl.equals("")) {
				printErrorMsgAndExit("\nFirst time run with output directory '" + 
						dbSrcDir.toString() + "'. Therefore --" + xsdURL + 
						"=url must be specifed");
			} else {
		...
		if (!bindingsFile.equals("")) {
			...
			File out	= new File(getAbsolutePath("xsd") + "/" + in.getName());
			if (in.canRead()) {
				try {
	...
	/**
	 * Creates absoulte paths for the database source
	 * output directory and any children directories
	 *
	 */
	private void createAbsoulutePaths() {
		...
		for (String dir: subDirs) {
			map.put(dir, new File(dbSrcDir.toString() + "/" + dir).getAbsolutePath());
		}
	 ...
	 * @throws Exception 	I/O error or File not found error	
	 */
	...
	private void downLoadXsdFile() throws IOException  {
		...
		File xsdfile = new File(getAbsolutePath("xsd") +
							xsdurl.substring(xsdurl.lastIndexOf("/")));
		URL url	 = null;
	...
	private void addXsd2dbOptions() {
		...
		options.addOption(OptionBuilder.withLongOpt( outputDirectory )
						  .withValueSeparator( '=' )
						  .hasArg()
						  .create());

		options.addOption(OptionBuilder.withLongOpt( bindings )
				  		  .withValueSeparator( '=' )
				  		  .hasArg()
				  		  .create());

		options.addOption(OptionBuilder.withLongOpt( xsdURL )
		  		  		  .withValueSeparator( '=' )
		  		          .hasArg()
		  		          .create());
	...
	private void createDirectoryStructure() {
		...
		for (String dir: subDirs) {
			new File(dbSrcDir.toString() + "/" + dir).mkdir();
		}
	}
